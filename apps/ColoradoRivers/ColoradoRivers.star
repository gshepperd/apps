...
def render_multi_station(stations_data, config, scale, is_wide, delay):
    ...
    # Adjust frame count for animation delay
    # At delay=50, 10 frames = 0.5 sec; at delay=25, 20 frames = 0.5 sec
    ...
    if not frames:
        return render.Root(
            delay = delay,
            child = render.Box(
                render.WrappedText("No stations configured", color = "#FF0000"),
            ),
        )
